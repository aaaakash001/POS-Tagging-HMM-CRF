...
def save_pickles(fold):
    ...
    with open(f'./demo_files/vocab.pkl', 'wb') as f:
        pickle.dump(vocab, f)

    with open(f'./demo_files/tags.pkl', 'wb') as f:
        pickle.dump(tags, f)

    with open(f'./demo_files/initial_probabilities.npy', 'wb') as f:
        np.save(f, initial_probabilities)

    with open(f'./demo_files/transition_matrix.npy', 'wb') as f:
        np.save(f, transition_matrix)

    with open(f'./demo_files/emission_matrix.npy', 'wb') as f:
...
def save_wrong_prediction(wrong_predictions, fold):
    ...
    csv_filename = f'wrong_predictions_fold_{fold + 1}.csv'
...
print(classification_report(all_true_tags, all_pred_tags, labels=tags, zero_division=0))
